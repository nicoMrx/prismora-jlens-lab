...
def test_v4_live_chat_intercepts_fake_composer_and_loads_real_artifact_without_reload():
    ...
    assert "loadArtifactIntoReader" in script
...
def test_v4_live_chat_has_honest_timed_progress_and_server_restore():
    script = (WEB / "v4-live-chat.js").read_text(encoding="utf-8")
    assert "Neuronpedia travaille sur votre message" in script
    assert "Le modèle génère sa réponse et calcule les readouts J-Lens." in script
    assert "Prismora attend les mesures de toutes les couches." in script
    assert "Toujours en cours" in script
    assert "setInterval(updateProgress, 1000)" in script
    assert "elapsedLabel" in script
    assert "progression est volontairement indéterminée" in script
    assert "LIVE_REF_KEY" in script
    assert "/api/runs/" in script
    assert "restoreLiveFromServer" in script
...
def test_v4_live_chat_assets_are_identical_between_source_and_package():
    for name in ("v4-live-chat.js", "v4-live-chat.css"):
...
def test_v4_route_injects_live_chat_after_connection_semantics_and_before_campaign(tmp_path):
    ...
    live_css = '/v4-live-chat.css?v=1'
    campaign = '/v4-campaign-center.js?v=2'
    assert connection in html and live in html and live_css in html and campaign in html
